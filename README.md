...
約 480 行程式（`src/` 不含註解與空行；連註解 841 行）、**零 runtime 依賴**、82 個測試。
...
| `["REQ", <subId>, <filter>]` | filter 只認 `kinds`、`since`、`#x`；**`kinds` 與 `#x` 是必填**，缺任一個整條 REQ 被拒；`since` 選填（預設 0）；一個 REQ 只能帶一個 filter |
| `["CLOSE", <subId>]` | 有 |
...
`src/limits.ts`**（不是另外手打一份；唯一的字面量是 `min_pow_difficulty: 0`，
因為「不做 PoW」沒有對應的上限常數）——一份會說謊的能力宣告比沒有更糟，客戶端會照它
...
| project | 跑在哪 | 守什麼 |
| --- | --- | --- |
| `unit` | node | 純函式葉檔，以及對 `relay.ts` 做**原始碼比對**的結構鎖 |
| `workers` | workerd | `test/relay.behaviour.test.ts` —— **真的把 Worker 與 RelayDO 跑起來**，斷言協定往返 |

第二個是後來補的。原本 `src/relay.ts` 從來沒有被任何測試執行過，只有字串比對守著，
而字串比對守的是拼字不是行為 —— 改個變數名就能把 fan-out 改成跳過發送者而整條 check 全綠。
...
要走 Git 自動部署（Workers Builds）的話，儀表板那五個欄位這樣填：

| 欄位 | 填什麼 | 為什麼 |
| --- | --- | --- |
| Worker name | `relay`（跟 `wrangler.jsonc` 的 `name` **一字不差**） | 不一樣會長出第二顆 Worker，而網域掛在空的那顆上 |
| Build command | **留空** | 沒有建置步驟，`wrangler deploy` 自己會打包 `src/` |
| Deploy command | `npx wrangler deploy` | |
| Build branches | **只設 production 分支**（`main`） | 非 production 分支跑 `wrangler deploy` 會紅，而且對 PR 沒有意義 |
| Root directory | 留空（repo 根目錄） | 單一套件，沒有 monorepo 子目錄 |
...
理由分寫在 `docs/spec.md` §7（協定層面）與 `docs/plan.md` §2（專案層面）。
